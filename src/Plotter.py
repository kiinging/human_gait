...
class Plotter:
    ...
    def __init__(self, data):
        ...
        self.ypr_lines = self.create_lines(self.axes[3], 3, labels=['yaw', 'pitch', 'roll'])
    ...
    def configure(self):
        ...
        self.axes[3].set_title('Orientation')
        for ax in self.axes:
            ax.legend()
        
        # Set y-axis limits limits for each subplot
        self.axes[0].set_ylim(-1.5, 1.5)
        self.axes[1].set_ylim(-501, 501)
        self.axes[2].set_ylim(-50, 50)
        self.axes[3].set_ylim(-200, 200)
    ...
    def animate(self, i):
        ...
        self.draw_lines(self.axes[3], self.ypr_lines, self.data.get_ypr())
        self.fig.canvas.draw()

    def create_lines(self, ax, num=3, labels=['x', 'y', 'z']):
        return [ax.plot([], [], label=labels[i])[0] for i in range(0, num)]

    def draw_lines(self, ax, lines, values):
        xs = list(range(max(0, len(values) - 100), len(values)))
        for idx in range(0, 3):
            ax.set_xlim(max(0, len(values) - 100), len(values))
            ys = list(map(lambda value: value[idx], values[max(0, len(values) - 100):]))
